...
it('assembly simple statement tiling tests', () => {
  ...
  expect(tileStatements([MIR_MOVE_IMMUTABLE_MEM(MIR_ONE, MIR_ZERO)])).toBe('mov qword ptr [1], 0');
  expect(
    tileStatements([MIR_MOVE_IMMUTABLE_MEM(MIR_ONE, MIR_CONST(Long.fromString('1000000000000')))])
  ).toBe(`movabs _ABSTRACT_REG_0, 1000000000000
mov qword ptr [1], _ABSTRACT_REG_0`);
  expect(tileStatements([MIR_MOVE_IMMUTABLE_MEM(MIR_ONE, MIR_IMMUTABLE_MEM(MIR_ONE))]))
    .toBe(`## MEM[1]
...
## We finished calling fff`);
  ...
  expect(tileStatements([MIR_CALL_FUNCTION(MIR_NAME('fff'), [])])).toBe(`## fff();
## We are about to call fff
call fff
## We finished calling fff`);
  expect(tileStatements([MIR_CALL_FUNCTION(MIR_NAME('fff'), [], 'result')]))
    .toBe(`## result = fff();
## We are about to call fff
call fff
mov result, rax
## We finished calling fff`);
  expect(tileStatements([MIR_CALL_FUNCTION(MIR_NAME('fff'), [MIR_ONE])])).toBe(`## fff(1);
## We are about to call fff
mov rdi, 1
call fff
## We finished calling fff`);
  expect(
    tileStatements([
      MIR_CALL_FUNCTION(MIR_NAME('fff'), [MIR_ONE, MIR_ONE, MIR_ONE, MIR_ONE, MIR_ONE, MIR_ONE]),
    ])
...
## We finished calling fff`);
  ...
  expect(
    tileStatements([
      MIR_CALL_FUNCTION(MIR_NAME('fff'), [
        MIR_ONE,
        MIR_ONE,
        MIR_ONE,
        MIR_ONE,
...
it('assembly constant tiling test', () => {
  expect(tileStatements([MIR_MOVE_TEMP('a', MIR_ONE)])).toBe('mov a, 1');
  expect(tileStatements([MIR_MOVE_TEMP('a', MIR_CONST(Long.fromString('1000000000000')))])).toBe(
    'movabs _ABSTRACT_REG_0, 1000000000000\nmov a, _ABSTRACT_REG_0'
  );
});

it('assembly name tiling test', () => {
  expect(tileStatements([MIR_MOVE_TEMP('_', MIR_NAME('hello-world'))]))
    .toBe(`lea _ABSTRACT_REG_0, qword ptr [rip+hello-world]
mov _, _ABSTRACT_REG_0`);
});

it('assembly memory tiling test', () => {
  expect(tileStatements([MIR_MOVE_TEMP('_', MIR_IMMUTABLE_MEM(MIR_IMMUTABLE_MEM(MIR_ONE)))]))
    .toBe(`## MEM[1]
mov _ABSTRACT_REG_0, qword ptr [1]
...
it('assembly comparison expressions tiling test', () => {
  expect(tileStatements([MIR_MOVE_TEMP('_', MIR_OP('<', MIR_ONE, MIR_ZERO))]))
...
it('assembly multiply by power of 2 tiling test', () => {
  expect(tileStatements([MIR_MOVE_TEMP('a', MIR_OP('*', MIR_ONE, MIR_CONST(65536)))]))
