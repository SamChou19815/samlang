...
pub(crate) fn type_check_source_handles(
  ...
  for (module_reference, source) in source_handles {
    let parsed =
      parse_source_module_from_text(&source, &module_reference, &mut heap, &mut error_set);
    parsed_sources.insert(module_reference, parsed);
  }
  let (checked_sources, global_typing_context) =
    type_check_sources(parsed_sources, &mut heap, &mut error_set);
  let mut compile_time_errors = vec![];
  for e in error_set.errors() {
    compile_time_errors.push(e.clone());
  }
  TypeCheckSourceHandlesResult { checked_sources, heap, global_typing_context, compile_time_errors }
}
...
pub(crate) fn type_check_single_module_source(
  ...
  type_check_sources(HashMap::from([(module_reference.clone(), module)]), heap, error_set)
    .0
