...
extern bool GC_init(void) {
  ...
  for (size_t i = 0; i < GC_NUM_REGIONS; i++) {
    ...
    if (offset != 0) startptr += size - offset;
...
nonempty_freelist:
  ...
  if (region->markstartptr < region->markendptr) {
    ...
    if (freelist != NULL) goto nonempty_freelist;
  }
...
static inline bool gc_mark_index(uint8_t *markptr_0, uint32_t idx) {
  ...
  if (markunit & markmask) return false;
...
gc_mark_loop_inner:
    ...
    while (ptrptr < endptr) {
      ...
      if (!gc_mark_index(region->markptr, ptridx)) continue;
...
static void gc_sweep(void) {
  ...
  for (size_t i = 0; i < GC_NUM_REGIONS; i++) {
    gc_region_t region = __gc_regions + i;
    if (region->freeptr == region->startptr) continue;
    void *ptr = region->freeptr - region->size;
    uint32_t size = region->size;
    uint8_t *markptr = region->markptr;

    if (i == GC_BIG_IDX_OFFSET) returning = true;
