...
GC_INLINE size_t GC_unit_offset(size_t unit) {
  return (unit == GC_UNIT? 0: (unit == GC_BIG_UNIT? GC_BIG_IDX_OFFSET: GC_HUGE_IDX_OFFSET));
}
 ...
 */
GC_INLINE size_t GC_index(void *ptr) {
  return ((size_t)ptr / GC_REGION_SIZE - ((size_t)GC_MEMORY / GC_REGION_SIZE));
}
...
GC_INLINE bool GC_isptr(const void *ptr) {
  ...
  return ((size_t)((char *)ptr - GC_MEMORY) < (size_t)GC_NUM_REGIONS*GC_REGION_SIZE);
}
...
GC_INLINE void *GC_malloc(size_t size) {
  ...
  if (size1 < GC_BIG_UNIT) idx = size1 / GC_UNIT;
