...
define i64 @_factorial(i64 %0) local_unnamed_addr nounwind {
  %comparison = icmp slt i64 %0, 2                ; if (%0 < 2) then goto loop_end else goto loop
  br i1 %comparison, label %loop_end, label %loop

loop:
  %i = phi i64 [ %i2, %loop ], [ %0, %1 ]         ; i = phi(i2 from loop, %0 from start)
  %acc = phi i64 [ %acc2, %loop ], [ 1, %1 ]      ; acc = phi(acc2 from loop, 1 from start)
...
loop_end:
  %result = phi i64 [ 1, %1 ], [ %acc2, %loop ]   ; result = phi(1 from start, acc2 from loop)
  ret i64 %result
}
