...
private fun loadPrograms(): List<TestProgram> {
    ...
    val programFiles: Array<File> = resourceFolder.listFiles() ?: error(message = "Test program folder not found.")
    return programFiles.mapNotNull { file ->
        file.takeIf { it.extension == "samlang" }?.let { f ->
            ...
            val type = when (val t = nameParts[0]) {
                ...
                "bad-type" -> TestProgramType.BAD_TYPE
                "bad-syntax" -> TestProgramType.BAD_SYNTAX
