...
private class ExpressionTypeCheckerVisitor(
    ...
    override fun visit(
        ...
        val (_, _, typeParameters, typeMappings) = ctx.getCurrentModuleTypeDefinition()
            ...
            ?: throw UnsupportedModuleTypeDefinitionError(typeDefinitionType = OBJECT, range = range)
        ...
        val locallyInferredType = if (checkedSpreadExpression != null) {
            ...
            val (genericsResolvedTypeMappings, autoGeneratedUndecidedTypes) = typeParameters?.let { params ->
    ...
    override fun visit(
        ...
        val (_, _, typeParameters, typeMappings) = ctx.getCurrentModuleTypeDefinition()
            ...
            ?: throw UnsupportedModuleTypeDefinitionError(typeDefinitionType = VARIANT, range = range)
        ...
        val locallyInferredType = IdentifierType(
            identifier = ctx.currentModule, typeArguments = constraintInferredTypeArgs
        )
        val constraintInferredType = constraintAwareTypeChecker.checkAndInfer(
            expectedType = expectedType, actualType = locallyInferredType, errorRange = range
        )
