...
internal class ExpressionTypeCheckerVisitor(private val manager: UndecidedTypeManager) : TypeCheckerVisitor {
    ...
    private fun Expression.toChecked(ctx: TypeCheckingContext, expectedType: TypeExpression): Expression =
        accept(visitor = this@ExpressionTypeCheckerVisitor, context = ctx to expectedType)

    private fun Expression.toChecked(ctx: TypeCheckingContext): Expression =
        accept(visitor = this@ExpressionTypeCheckerVisitor, context = ctx to type)
    ...
    override fun visit(
        ...
        val locallyInferredType = if (checkedSpreadExpression != null) {
            ...
            val constraintInferredTypeArguments = autoGeneratedUndecidedTypes?.map { undecidedType ->
                manager.reportCurrentUndecidedTypeReference(undecidedType = undecidedType)
            }
            IdentifierType(
                range = range,
    ...
    override fun visit(
        ...
        val (_, typeParams, typeMappings) = ctx.getCurrentModuleVariantTypeDef(errorRange = range)
        ...
        val (genericsResolvedAssociatedDataType, autoGeneratedUndecidedTypes) = typeParams?.let { params ->
            CheckedTypeDeparameterizer.convert(
                typeExpression = associatedDataType,
                typeParameters = params
            )
