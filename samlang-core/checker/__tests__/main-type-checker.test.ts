...
import { AstBuilder, SamlangType } from "../../ast/samlang-nodes";
...
function typeCheckInSandbox(
  ...
  // Type Check
  typeCheckExpression(parsedExpression, context, expectedType);
...
function assertTypeChecks(source: string, expectedType: SamlangType, currentClass?: string): void {
  expect(typeCheckInSandbox(source, expectedType, currentClass)).toEqual([]);
}
...
function assertTypeErrors(
  ...
  expect(typeCheckInSandbox(source, expectedType, currentClass)).toEqual(expectedErrors);
}
...
describe("main-type-checker", () => {
  ...
  it("VariantConstructor", () => {
    assertTypeChecks("Test2.Foo(true)", AstBuilder.IdType("Test2"), "Test2");
    assertTypeChecks("Test2.Bar(42)", AstBuilder.IdType("Test2"), "Test2");
    assertTypeChecks("Test4.Foo(true)}", AstBuilder.IdType("Test4", [bool]), "Test4");
    assertTypeChecks("Test4.Foo<bool>(true)}", AstBuilder.IdType("Test4", [bool]), "Test4");
  ...
  describe("StatementBlocks", () => {
    it("Object destructuring 1", () =>
      assertTypeChecks("{val {a, b as c} = A.init();}", unit, "A"));
